...
class DataFramer:
    ...
    def visit_batch_set(self, batch_set: BatchSet):
        ...
        """
        arch = get_archive()
        for batch, news_metadata in zip(batch_set.batches, batch_set.metadatas):
            articles = arch.get_articles(batch)
            df = pd.DataFrame(
                {
                    "doc": [a.story for a in articles],
                    "uri": [a.path() for a in articles],
                    "section0": [
                        meta["section0"] or "front-page" for meta in news_metadata
                    ],
                    "headline0": [meta["headline0"] for meta in news_metadata],
                    "section1": [meta.get("section1") for meta in news_metadata],
                    "headline1": [meta.get("headline1") for meta in news_metadata],
                    "section2": [meta.get("section2") for meta in news_metadata],
                    "headline2": [meta.get("headline2") for meta in news_metadata],
                    "answer": "",
                    "question": "",
                    "train_question": "",
                    "prompt": "",
                    "ne": [meta["ner_terms"] for meta in news_metadata],
                }
            )
